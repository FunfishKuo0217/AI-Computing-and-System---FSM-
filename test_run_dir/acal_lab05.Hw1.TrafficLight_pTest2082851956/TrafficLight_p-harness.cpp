...
#endif
#include <iostream>
class TrafficLight_p_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    TrafficLight_p_api_t(VTrafficLight_p* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_P_button)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_timer)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_P_traffic)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_V_traffic)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_H_traffic)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["TrafficLight_p.reset"] = 0;
    }
#if VM_TRACE
...
static TrafficLight_p_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VTrafficLight_p* top = new VTrafficLight_p;
    std::string vcdfile = "test_run_dir/acal_lab05.Hw1.TrafficLight_pTest2082851956/TrafficLight_p.vcd";
